...
xterm -fg white -bg black -l -lf $logs_directory/federation-manager-${timestamp}.log -T "Federation Manager" -geometry 140x40+0+30 -e "export CPSWT_ROOT=`pwd` && mvn exec:java" &
...
cd $root_directory/Auction
xterm -fg yellow -bg black -l -lf $logs_directory/Auction-${timestamp}.log -T "Auction" -geometry 140x40+30+60 -e "sh run.sh conf/Auction-RTP.json" &
waitUntilJoined Auction 1

cd $root_directory/FileReader
xterm -fg green -bg black -l -lf $logs_directory/FileReader-${timestamp}.log -T "FileReader" -geometry 140x40+60+90 -e "sh run.sh conf/FileReader-RTP-July6-to-July7.json" &
waitUntilJoined FileReader 1

cd $root_directory/GridModel
...
cd $root_directory/SimulationTime
xterm -fg white -bg black -l -lf $logs_directory/SimulationTime-${timestamp}.log -T "SimulationTime" -geometry 140x40+120+150 -e "sh run.sh conf/SimulationTime-July6-to-July7.json" &
