...
xterm -fg white -bg black -l -lf $logs_directory/federation-manager-${timestamp}.log -T "Federation Manager" -geometry 140x40+0+30 -e "export CPSWT_ROOT=`pwd` && mvn exec:java -Dexec.args=\"-configFile conf/marketconfig.json\"" &
...
cd $root_directory/generated/SimulationTime
xterm -fg yellow -bg black -l -lf $logs_directory/SimulationTime-${timestamp}.log -T "SimulationTime" -geometry 140x40+30+60 -e "sh run.sh conf/SimulationTimeWithMarket.json" &
waitUntilJoined SimulationTime 1

cd $root_directory/generated/GridModel
xterm -fg green -bg black -l -lf $logs_directory/GridModel-${timestamp}.log -T "GridModel" -geometry 140x40+60+90 -e "sh run.sh conf/GridModelEV.json" &
waitUntilJoined GridModel 1

cd $root_directory/generated/FlexibleResourceController
xterm -fg cyan -bg black -l -lf $logs_directory/Controller-${timestamp}.log -T "FlexibleResourceController" -geometry 140x40+90+120 -e "sh run.sh conf/FlexibleResourceControllerTEM.json" &
...
cd $root_directory/generated/MarketAgent
xterm -fg purple -bg white -l -lf $logs_directory/MarketAgent-${timestamp}.log -T "MarketAgent" -geometry 140x40+150+180 -e "sh run.sh" &
waitUntilJoined MarketAgent 1

cd $root_directory/generated/UserAgent
xterm -fg blue -bg white -l -lf $logs_directory/UserAgent-${timestamp}.log -T "UserAgent" -geometry 140x40+180+210 -e "sh run.sh" &
waitUntilJoined UserAgent 1
