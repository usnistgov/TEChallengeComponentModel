...
from pypower_iso.qps_pips_iso import qps_pips_iso
from pypower_iso.qps_ipopt_iso import qps_ipopt_iso
from pypower_iso.qps_cplex_iso import qps_cplex_iso
from pypower_iso.qps_mosek_iso import qps_mosek_iso
from pypower_iso.qps_gurobi_iso import qps_gurobi_iso

from pypower_iso.util_iso import have_fcn_iso


def qps_pypower_iso(H, c=None, A=None, l=None, u=None, xmin=None, xmax=None,
                x0=None, opt=None):
    """Quadratic Program Solver for PYPOWER.
...
#        xmin = array([])
    ...
    if alg == 0:
        if have_fcn_iso('cplex'):        ## use CPLEX by default, if available
            ...
            alg = 600
        elif have_fcn_iso('gurobipy'):   ## if not, then Gurobi, if available
            alg = 700
        else:                        ## otherwise PIPS
            alg = 200
    ...
    if alg == 200 or alg == 250:    ## use MIPS or sc-MIPS
        ...
        x, f, eflag, output, lmbda = \
            qps_pips_iso(H, c, A, l, u, xmin, xmax, x0, pips_opt)
    elif alg == 400:                    ## use IPOPT
        x, f, eflag, output, lmbda = \
            qps_ipopt_iso(H, c, A, l, u, xmin, xmax, x0, opt)
    elif alg == 500:                    ## use CPLEX
        x, f, eflag, output, lmbda = \
            qps_cplex_iso(H, c, A, l, u, xmin, xmax, x0, opt)
    elif alg == 600:                    ## use MOSEK
        x, f, eflag, output, lmbda = \
            qps_mosek_iso(H, c, A, l, u, xmin, xmax, x0, opt)
    elif 700:                           ## use Gurobi
        x, f, eflag, output, lmbda = \
            qps_gurobi_iso(H, c, A, l, u, xmin, xmax, x0, opt)
    else:
