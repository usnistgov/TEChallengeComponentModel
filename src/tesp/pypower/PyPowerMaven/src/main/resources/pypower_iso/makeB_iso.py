...
from pypower_iso.idx_bus_iso import BS
from pypower_iso.idx_brch_iso import BR_B, BR_R, TAP, SHIFT

from pypower_iso.makeYbus_iso import makeYbus_iso


def makeB_iso(baseMVA, bus, branch, alg):
    """Builds the FDPF matrices, B prime and B double prime.
    ...
    Bp = -1 * makeYbus_iso(baseMVA, temp_bus, temp_branch)[0].imag
    ...
    if alg == 3:                               ## if BX method
        temp_branch[:, BR_R] = zeros(nl)    ## zero out line resistance
    Bpp = -1 * makeYbus_iso(baseMVA, bus, temp_branch)[0].imag
