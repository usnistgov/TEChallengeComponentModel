...
from pypower_iso.dcopf_solver_iso import dcopf_solver_iso
from pypower_iso.pipsopf_solver_iso import pipsopf_solver_iso
from pypower_iso.ipoptopf_solver_iso import ipoptopf_solver_iso
from pypower_iso.update_mupq_iso import update_mupq_iso
...
from pypower_iso.idx_bus_iso import VM
from pypower_iso.idx_gen_iso import GEN_BUS, VG
from pypower_iso.idx_brch_iso import MU_ANGMIN, MU_ANGMAX


def opf_execute_iso(om, ppopt):
    """Executes the OPF specified by an OPF model object.
    ...
    if dc:
        ...
        results, success, raw = dcopf_solver_iso(om, ppopt)
    else:
        ##-----  run AC OPF solver  -----
        if verbose > 0:
        ...
        if alg == 560 or alg == 565:                   ## PIPS
            results, success, raw = pipsopf_solver_iso(om, ppopt)
        elif alg == 580:                              ## IPOPT
            try:
                __import__('pyipopt')
                results, success, raw = ipoptopf_solver_iso(om, ppopt)
            except ImportError:
    ...
    if success:
        if not dc:
            ...
            if (ll['N']['PQh'] > 0) | (ll['N']['PQl'] > 0):
                ...
                results['gen'] = update_mupq_iso(results['baseMVA'], results['gen'], mu_PQh, mu_PQl, Apqdata)
            ...
            if ppopt['RETURN_RAW_DER']:
                ...
                if 'dg' not in raw:
                    ppc = om.get_ppc()
                    Ybus, Yf, Yt = makeYbus_iso(ppc['baseMVA'], ppc['bus'], ppc['branch'])
                    g, geq, dg, dgeq = opf_consfcn_iso(results['x'], om, Ybus, Yf, Yt, ppopt)
                    raw['g'] = r_[geq, g]
                    raw['dg'] = r_[dgeq.T, dg.T]   ## true Jacobian organization

                ## compute df, d2f
                _, df, d2f = opf_costfcn_iso(results['x'], om, True)
                raw['df'] = df
                raw['d2f'] = d2f
