...
from pypower_iso.idx_gen_iso import PG, QG
from pypower_iso.idx_brch_iso import F_BUS, T_BUS
from pypower_iso.idx_cost_iso import MODEL, POLYNOMIAL

from pypower_iso.polycost_iso import polycost_iso
from pypower_iso.d2Sbus_dV2_iso import d2Sbus_dV2_iso
from pypower_iso.dSbr_dV_iso import dSbr_dV_iso
from pypower_iso.dIbr_dV_iso import dIbr_dV_iso
from pypower_iso.d2AIbr_dV2_iso import d2AIbr_dV2_iso
from pypower_iso.d2ASbr_dV2_iso import d2ASbr_dV2_iso
from pypower_iso.opf_costfcn_iso import opf_costfcn_iso
from pypower_iso.opf_consfcn_iso import opf_consfcn_iso


def opf_hessfcn_iso(x, lmbda, om, Ybus, Yf, Yt, ppopt, il=None, cost_mult=1.0):
    """Evaluates Hessian of Lagrangian for AC OPF.
...
#                  (i, i)), shape=(nxyz, nxyz))
    ...
    Gpaa, Gpav, Gpva, Gpvv = d2Sbus_dV2_iso(Ybus, V, lamP)
    Gqaa, Gqav, Gqva, Gqvv = d2Sbus_dV2_iso(Ybus, V, lamQ)
    ...
    if ppopt['OPF_FLOW_LIM'] == 2:       ## current
        dIf_dVa, dIf_dVm, dIt_dVa, dIt_dVm, If, It = dIbr_dV_iso(branch, Yf, Yt, V)
        Hfaa, Hfav, Hfva, Hfvv = d2AIbr_dV2_iso(dIf_dVa, dIf_dVm, If, Yf, V, muF)
        Htaa, Htav, Htva, Htvv = d2AIbr_dV2_iso(dIt_dVa, dIt_dVm, It, Yt, V, muT)
    else:
        ...
        dSf_dVa, dSf_dVm, dSt_dVa, dSt_dVm, Sf, St = \
                dSbr_dV_iso(branch[il,:], Yf, Yt, V)
        if ppopt['OPF_FLOW_LIM'] == 1:     ## real power
            Hfaa, Hfav, Hfva, Hfvv = d2ASbr_dV2_iso(dSf_dVa.real, dSf_dVm.real,
                                                Sf.real, Cf, Yf, V, muF)
            Htaa, Htav, Htva, Htvv = d2ASbr_dV2_iso(dSt_dVa.real, dSt_dVm.real,
                                                St.real, Ct, Yt, V, muT)
        else:                  ## apparent power
            Hfaa, Hfav, Hfva, Hfvv = \
                    d2ASbr_dV2_iso(dSf_dVa, dSf_dVm, Sf, Cf, Yf, V, muF)
            Htaa, Htav, Htva, Htvv = \
                    d2ASbr_dV2_iso(dSt_dVa, dSt_dVm, St, Ct, Yt, V, muT)
    ...
    if 0:
        ...
        for i in range(nx):
            ...
            _, dfp = opf_costfcn_iso(xp, om)
            _, dfm = opf_costfcn_iso(xm, om)
            # evaluate constraints & gradients
            _, _, dHp, dGp = opf_consfcn_iso(xp, om, Ybus, Yf, Yt, ppopt, il)
            _, _, dHm, dGm = opf_consfcn_iso(xm, om, Ybus, Yf, Yt, ppopt, il)
