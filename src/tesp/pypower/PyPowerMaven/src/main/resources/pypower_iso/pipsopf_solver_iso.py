...
from pypower_iso.idx_bus_iso import BUS_TYPE, REF, VM, VA, MU_VMAX, MU_VMIN, LAM_P, LAM_Q
from pypower_iso.idx_brch_iso import F_BUS, T_BUS, RATE_A, PF, QF, PT, QT, MU_SF, MU_ST
from pypower_iso.idx_gen_iso import GEN_BUS, PG, QG, VG, MU_PMAX, MU_PMIN, MU_QMAX, MU_QMIN
from pypower_iso.idx_cost_iso import MODEL, PW_LINEAR, NCOST
...
from pypower_iso.opf_hessfcn_iso import opf_hessfcn_iso
from pypower_iso.pips_iso import pips_iso
from pypower_iso.util_iso import sub2ind_iso

def pipsopf_solver_iso(om, ppopt, out_opt=None):
    """Solves AC optimal power flow using PIPS.
...
#         c = totcost(gencost[ipwl, :], PQ[ipwl])
        c = gencost.flatten('F')[sub2ind_iso(gencost.shape, ipwl, NCOST+2*gencost[ipwl, NCOST])]    ## largest y-value in CCV data
...
#        x0[vv["i1"]["y"]:vv["iN"]["y"]] = c + 0.1 * abs(c)
    ...
    ##-----  run opf  -----
    f_fcn = lambda x, return_hessian=False: opf_costfcn_iso(x, om, return_hessian)
    gh_fcn = lambda x: opf_consfcn_iso(x, om, Ybus, Yf[il, :], Yt[il,:], ppopt, il)
    hess_fcn = lambda x, lmbda, cost_mult: opf_hessfcn_iso(x, lmbda, om, Ybus, Yf[il, :], Yt[il, :], ppopt, il, cost_mult)

    solution = pips_iso(f_fcn, x0, A, l, u, xmin, xmax, gh_fcn, hess_fcn, opt)
