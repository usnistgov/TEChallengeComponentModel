...
from pypower_iso.idx_brch_iso import PF


def toggle_iflims_iso(ppc, on_off):
    """Enable or disable set of interface flow constraints.
    ...
    if on_off == 'on':
        ...
        ppc = add_userfcn_iso(ppc, 'ext2int', userfcn_iflims_ext2int)
        ppc = add_userfcn_iso(ppc, 'formulation', userfcn_iflims_formulation)
        ppc = add_userfcn_iso(ppc, 'int2ext', userfcn_iflims_int2ext)
        ppc = add_userfcn_iso(ppc, 'printpf', userfcn_iflims_printpf)
        ppc = add_userfcn_iso(ppc, 'savecase', userfcn_iflims_savecase)
    elif on_off == 'off':
        ppc = remove_userfcn_iso(ppc, 'savecase', userfcn_iflims_savecase)
        ppc = remove_userfcn_iso(ppc, 'printpf', userfcn_iflims_printpf)
        ppc = remove_userfcn_iso(ppc, 'int2ext', userfcn_iflims_int2ext)
        ppc = remove_userfcn_iso(ppc, 'formulation', userfcn_iflims_formulation)
        ppc = remove_userfcn_iso(ppc, 'ext2int', userfcn_iflims_ext2int)
    else:
...
def userfcn_iflims_formulation(om, *args):
    ...
    _, Bf, _, Pfinj = makeBdc_iso(baseMVA, bus, branch)
