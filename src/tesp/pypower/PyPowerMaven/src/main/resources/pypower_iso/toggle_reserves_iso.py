...
from pypower_iso.e2i_field_iso import e2i_field_iso
from pypower_iso.i2e_field_iso import i2e_field_iso
from pypower_iso.i2e_data_iso import i2e_data_iso
...
from pypower_iso.add_userfcn_iso import add_userfcn_iso
from pypower_iso.remove_userfcn_iso import remove_userfcn_iso
from pypower_iso.ext2int_iso import ext2int_iso
from pypower_iso.int2ext_iso import int2ext_iso
from pypower_iso.idx_gen_iso import RAMP_10, PMAX, GEN_STATUS, GEN_BUS


def toggle_reserves_iso(ppc, on_off):
    """Enable or disable fixed reserve requirements.
    ...
    if on_off == 'on':
        ...
        ppc = add_userfcn_iso(ppc, 'ext2int', userfcn_reserves_ext2int)
        ppc = add_userfcn_iso(ppc, 'formulation', userfcn_reserves_formulation)
        ppc = add_userfcn_iso(ppc, 'int2ext', userfcn_reserves_int2ext)
        ppc = add_userfcn_iso(ppc, 'printpf', userfcn_reserves_printpf)
        ppc = add_userfcn_iso(ppc, 'savecase', userfcn_reserves_savecase)
    elif on_off == 'off':
        ppc = remove_userfcn_iso(ppc, 'savecase', userfcn_reserves_savecase)
        ppc = remove_userfcn_iso(ppc, 'printpf', userfcn_reserves_printpf)
        ppc = remove_userfcn_iso(ppc, 'int2ext', userfcn_reserves_int2ext)
        ppc = remove_userfcn_iso(ppc, 'formulation', userfcn_reserves_formulation)
        ppc = remove_userfcn_iso(ppc, 'ext2int', userfcn_reserves_ext2int)
    else:
...
def userfcn_reserves_ext2int(ppc, *args):
    ...
    if 'qty' in r:
        ppc = e2i_field_iso(ppc, ['reserves', 'qty'], 'gen')

    ppc = e2i_field_iso(ppc, ['reserves', 'cost'], 'gen')
    ppc = e2i_field_iso(ppc, ['reserves', 'zones'], 'gen', 1)
    ppc = e2i_field_iso(ppc, ['reserves', 'rgens'], 'gen', 1)
...
def userfcn_reserves_int2ext(results, *args):
    ...
    ## convert all reserve parameters (zones, costs, qty, rgens)
    if 'qty' in r:
        results = i2e_field_iso(results, ['reserves', 'qty'], ordering='gen')

    results = i2e_field_iso(results, ['reserves', 'cost'], ordering='gen')
    results = i2e_field_iso(results, ['reserves', 'zones'], ordering='gen', dim=1)
    results = i2e_field_iso(results, ['reserves', 'rgens'], ordering='gen', dim=1)
    ...
    z = zeros(ng0)
    results['reserves']['R']       = i2e_data_iso(results, R, z, 'gen')
    results['reserves']['Rmin']    = i2e_data_iso(results, Rmin, z, 'gen')
    results['reserves']['Rmax']    = i2e_data_iso(results, Rmax, z, 'gen')
    if 'mu' not in results['reserves']:
        results['reserves']['mu'] = {}
    results['reserves']['mu']['l']    = i2e_data_iso(results, mu_l, z, 'gen')
    results['reserves']['mu']['u']    = i2e_data_iso(results, mu_u, z, 'gen')
    results['reserves']['mu']['Pmax'] = i2e_data_iso(results, mu_Pmax, z, 'gen')
