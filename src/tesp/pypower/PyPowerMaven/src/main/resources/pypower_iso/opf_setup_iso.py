...
from pypower_iso.pqcost_iso import pqcost_iso
from pypower_iso.opf_args_iso import opf_args_iso
from pypower_iso.makeBdc_iso import makeBdc_iso
from pypower_iso.makeAvl_iso import makeAvl_iso
from pypower_iso.makeApq_iso import makeApq_iso
from pypower_iso.makeAang_iso import makeAang_iso
from pypower_iso.makeAy_iso import makeAy_iso
from pypower_iso.opf_model_iso import opf_model_iso
from pypower_iso.run_userfcn_iso import run_userfcn_iso

from pypower_iso.idx_cost_iso import MODEL, NCOST, PW_LINEAR, COST, POLYNOMIAL
from pypower_iso.idx_bus_iso import BUS_TYPE, REF, VA, VM, PD, GS, VMAX, VMIN
from pypower_iso.idx_gen_iso import GEN_BUS, VG, PG, QG, PMAX, PMIN, QMAX, QMIN
from pypower_iso.idx_brch_iso import RATE_A


def opf_setup_iso(ppc, ppopt):
    """Constructs an OPF model object from a PYPOWER case dict.
    ...
    if dc:
        ...
        ppc['gencost'], _ = pqcost_iso(ppc['gencost'], ng)
    ...
    baseMVA, bus, gen, branch, gencost, _, lbu, ubu, ppopt, \
            _, fparm, H, Cw, z0, zl, zu, userfcn, _ = opf_args_iso(ppc, ppopt)
    ...
    if dc:               ## DC model
        ...
        Avl, lvl, uvl, _  = makeAvl_iso(baseMVA, gen)

        ## generator PQ capability curve constraints
        Apqh, ubpqh, Apql, ubpql, Apqdata = makeApq_iso(baseMVA, gen)
    ...
    Aang, lang, uang, iang  = makeAang_iso(baseMVA, branch, nb, ppopt)
    ...
    if alg == 545 or alg == 550:     ## SC-PDIPM or TRALM, no CCV cost vars
        ...
        Ay, by = makeAy_iso(baseMVA, ng, gencost, 1, q1, 1+ng+nq)
...
#        if len(H) > 0:
    ...
    run_userfcn_iso(userfcn, 'formulation', om)
