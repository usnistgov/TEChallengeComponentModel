...
from pypower_iso.idx_bus_iso import PQ, PV, REF, NONE, BUS_I, BUS_TYPE
from pypower_iso.idx_gen_iso import GEN_BUS, GEN_STATUS
from pypower_iso.idx_brch_iso import F_BUS, T_BUS, BR_STATUS
from pypower_iso.idx_area_iso import PRICE_REF_BUS

from pypower_iso.e2i_field_iso import e2i_field_iso
from pypower_iso.e2i_data_iso import e2i_data_iso

from pypower_iso.run_userfcn_iso import run_userfcn_iso


def ext2int_iso(ppc, val_or_field=None, ordering=None, dim=0):
    """Converts external to internal indexing.
...
#                ppc["areas"][o["areas"]["status"]["off"], :] = array([])
            ...
            if 'gencost' in ppc:
                ...
                ppc = e2i_field_iso(ppc, 'gencost', ordering)
            ...
            if 'A' in ppc:
                ppc = e2i_field_iso(ppc, 'A', ordering, 1)
            if 'N' in ppc:
                ppc = e2i_field_iso(ppc, 'N', ordering, 1)

            ## execute userfcn callbacks for 'ext2int' stage
            if 'userfcn' in ppc:
                ppc = run_userfcn_iso(ppc['userfcn'], 'ext2int', ppc)
        ...
        if isinstance(val_or_field, str) or isinstance(val_or_field, list):
            ...
            ppc = e2i_field_iso(ppc, gen, branch, dim)
            ...
            warn('Calls of the form val = ext2int(ppc, val, ...) have been '
                 ...
                 DeprecationWarning)
            gen, branch = val_or_field, ordering
            ppc = e2i_data_iso(ppc, gen, branch, dim)
