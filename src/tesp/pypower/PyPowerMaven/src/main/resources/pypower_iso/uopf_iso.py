...
from pypower_iso.fairmax_iso import fairmax_iso
from pypower_iso.opf_iso import opf_iso

from pypower_iso.idx_bus_iso import PD
from pypower_iso.idx_gen_iso import GEN_STATUS, PG, QG, PMIN, MU_PMIN
...
def uopf_iso(*args):
    ...
    ppc, ppopt = opf_args2_iso(*args)

    ## options
    verbose = ppopt["VERBOSE"]
    if verbose:      ## turn down verbosity one level for calls to opf
        ppopt = ppoption_iso(ppopt, VERBOSE=verbose - 1)

    ##-----  do combined unit commitment/optimal power flow  -----

    ## check for sum(Pmin) > total load, decommit as necessary
    on   = find( (ppc["gen"][:, GEN_STATUS] > 0) & ~isload_iso(ppc["gen"]) )   ## gens in service
    onld = find( (ppc["gen"][:, GEN_STATUS] > 0) &  isload_iso(ppc["gen"]) )   ## disp loads in serv
    ...
    while sum(Pmin) > load_capacity:
        ...
        avgPmincost = totcost_iso(ppc["gencost"][on, :], Pmin) / Pmin
        _, i = fairmax_iso(avgPmincost)   ## pick one with max avg cost at Pmin
