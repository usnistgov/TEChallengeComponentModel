...
def ipoptopf_solver_iso(om, ppopt):
    """Solves AC optimal power flow using IPOPT.
...
#        c = totcost(gencost[ipwl, :], PQ[ipwl])
        ...
        c = gencost.flatten('F')[sub2ind_iso(shape(gencost), ipwl, NCOST + 2 * gencost[ipwl, NCOST])]
...
#        x0[vv['i1']['y']:vv['iN']['y']) = c + 0.1 * abs(c)
    ...
    f, _, d2f = opf_costfcn_iso(x0, om, True)
    Hs = tril(d2f + vstack([
...
def eval_g(x, user_data=None):
    ...
    hn, gn, _, _ = opf_consfcn_iso(x, om, Ybus, Yf, Yt, ppopt, il)
...
def eval_jac_g(x, flag, user_data=None):
    ...
    if flag:
        ...
        _, _, dhn, dgn = opf_consfcn_iso(x, om, Ybus, Yf, Yt, ppopt, il)
...
def eval_h(x, lagrange, obj_factor, flag, user_data=None):
    ...
    if flag:
        ...
        H = opf_hessfcn_iso(x, lam, om, Ybus, Yf, Yt, ppopt, il, obj_factor)
