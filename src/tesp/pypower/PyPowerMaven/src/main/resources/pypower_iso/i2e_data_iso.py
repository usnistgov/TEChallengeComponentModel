...
from pypower_iso.get_reorder_iso import get_reorder_iso
from pypower_iso.set_reorder_iso import set_reorder_iso


def i2e_data_iso(ppc, val, oldval, ordering, dim=0):
    ...
    if isinstance(ordering, str):         ## single set
        if ordering == 'gen':
            v = get_reorder_iso(val, o[ordering]["i2e"], dim)
        else:
            v = val
        val = set_reorder_iso(oldval, v, o[ordering]["status"]["on"], dim)
    else:                                 ## multiple sets
        ...
        for ordr in ordering:
            ...
            ni = ppc[ordr].shape[0]
            v = get_reorder_iso(val, bi + arange(ni), dim)
            oldv = get_reorder_iso(oldval, be + arange(ne), dim)
            new_v.append( int2ext(ppc, v, oldv, ordr, dim) )
            be = be + ne
            bi = bi + ni
        ni = val.shape[dim]
        if ni > bi:              ## the rest
            v = get_reorder_iso(val, arange(bi, ni), dim)
            new_v.append(v)
