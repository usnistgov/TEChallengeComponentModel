...
from pypower_iso.idx_bus_iso import BUS_I
from pypower_iso.idx_gen_iso import GEN_BUS
...
def int2ext_iso(ppc, val_or_field=None, oldval=None, ordering=None, dim=0):
    """Converts internal to external bus numbering.
    ...
    if val_or_field is None: # nargin == 1
        ...
        if o["state"] == 'i':
            ...
            if 'userfcn' in ppc:
                ppc = run_userfcn_iso(ppc["userfcn"], 'int2ext', ppc)
        ...
        if isinstance(val_or_field, str) or isinstance(val_or_field, list):
            ...
            if ordering is not None:
                dim = ordering
            ppc = i2e_field_iso(ppc, bus, gen, dim)
        else:
            ## value
            ...
            ppc = i2e_data_iso(ppc, bus, gen, branch, dim)
