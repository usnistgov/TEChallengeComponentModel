...
def e2i_data_iso(ppc, val, ordering, dim=0):
    """Converts data from external to internal indexing.
    ...
    if isinstance(ordering, str):        ## single set
        ...
        val = get_reorder_iso(val, idx, dim)
    else:                            ## multiple: sets
        b = 0  ## base
        new_v = []
        for ordr in ordering:
            n = o["ext"][ordr].shape[0]
            v = get_reorder_iso(val, b + arange(n), dim)
            new_v.append( e2i_data_iso(ppc, v, ordr, dim) )
            b = b + n
        n = val.shape[dim]
        if n > b:                ## the rest
            v = get_reorder_iso(val, arange(b, n), dim)
            new_v.append(v)
