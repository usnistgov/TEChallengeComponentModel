...
from pypower_iso.toggle_reserves_iso import toggle_reserves_iso
from pypower_iso.runopf_iso import runopf_iso


def runopf_w_res_iso(*args):
    """Runs an optimal power flow with fixed zonal reserves.
    ...
    """
    ppc = loadcase_iso(args[0])
    ppc = toggle_reserves_iso(ppc, 'on')

    r = runopf_iso(ppc, *args[1:])
    r = toggle_reserves_iso(r, 'off')
