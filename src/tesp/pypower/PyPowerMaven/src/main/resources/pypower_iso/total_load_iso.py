...
from pypower_iso.idx_bus_iso import PD, QD, BUS_AREA, BUS_I
from pypower_iso.idx_gen_iso import QMAX, QMIN, GEN_BUS, GEN_STATUS, PMIN
...
def total_load_iso(bus, gen=None, load_zone=None, which_type=None):
    ...
    if want_disp:            ## need dispatchable
        ng = gen.shape[0]
        is_ld = isload_iso(gen) & (gen[:, GEN_STATUS] > 0)
        ld = find(is_ld)
