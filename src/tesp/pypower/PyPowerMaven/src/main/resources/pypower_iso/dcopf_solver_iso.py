...
from pypower_iso.idx_bus_iso import BUS_TYPE, REF, VA, LAM_P, LAM_Q, MU_VMAX, MU_VMIN
from pypower_iso.idx_gen_iso import PG, MU_PMAX, MU_PMIN, MU_QMAX, MU_QMIN
from pypower_iso.idx_brch_iso import PF, PT, QF, QT, RATE_A, MU_SF, MU_ST
from pypower_iso.idx_cost_iso import MODEL, POLYNOMIAL, PW_LINEAR, NCOST, COST

from pypower_iso.util_iso import sub2ind_iso, have_fcn_iso
from pypower_iso.ipopt_options_iso import ipopt_options_iso
from pypower_iso.cplex_options_iso import cplex_options_iso
from pypower_iso.mosek_options_iso import mosek_options_iso
from pypower_iso.gurobi_options_iso import gurobi_options_iso
from pypower_iso.qps_pypower_iso import qps_pypower_iso


def dcopf_solver_iso(om, ppopt, out_opt=None):
    """Solves a DC optimal power flow.
    ...
    if alg == 0:
        if have_fcn_iso('cplex'):        ## use CPLEX by default, if available
            alg = 500
        elif have_fcn_iso('mosek'):      ## if not, then MOSEK, if available
            alg = 600
        elif have_fcn_iso('gurobi'):     ## if not, then Gurobi, if available
            alg = 700
        else:                        ## otherwise PIPS
            alg = 200
    ...
    if (alg == 200) or (alg == 250):
        ...
        if ny > 0:
            ...
            c = gencost.flatten('F')[sub2ind_iso(gencost.shape, ipwl,
        ...
        opt['ipopt_opt'] = ipopt_options_iso([], ppopt)
    elif alg == 500:
        opt['cplex_opt'] = cplex_options_iso([], ppopt)
    elif alg == 600:
        opt['mosek_opt'] = mosek_options_iso([], ppopt)
    elif alg == 700:
        opt['grb_opt'] = gurobi_options_iso([], ppopt)
    else:
        raise ValueError("Unrecognised solver [%d]." % alg)

    ##-----  run opf  -----
    x, f, info, output, lmbda = \
            qps_pypower_iso(HH, CC, A, l, u, xmin, xmax, x0, opt)
    success = (info == 1)
