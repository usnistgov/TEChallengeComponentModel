...
from pypower_iso.idx_bus_iso import BUS_I, BUS_TYPE, PD, QD, GS, BS, BUS_AREA, \
    VM, VA, VMAX, VMIN, LAM_P, LAM_Q, MU_VMAX, MU_VMIN, REF
from pypower_iso.idx_gen_iso import GEN_BUS, PG, QG, QMAX, QMIN, GEN_STATUS, \
    PMAX, PMIN, MU_PMAX, MU_PMIN, MU_QMAX, MU_QMIN
from pypower_iso.idx_brch_iso import F_BUS, T_BUS, BR_R, BR_X, BR_B, RATE_A, \
    TAP, SHIFT, BR_STATUS, PF, QF, PT, QT, MU_SF, MU_ST

from pypower_iso.isload_iso import isload_iso
from pypower_iso.run_userfcn_iso import run_userfcn_iso
from pypower_iso.ppoption_iso import ppoption_iso


def printpf_iso(baseMVA, bus=None, gen=None, branch=None, f=None, success=None,
    ...
    if isinstance(baseMVA, dict):
        ...
        results = baseMVA
        if gen is None:
            ppopt = ppoption_iso()   ## use default options
        else:
            ppopt = gen
        if (ppopt['OUT_ALL'] == 0):
    ...
    allg = find( ~isload_iso(gen) )
    ong  = find( (gen[:, GEN_STATUS] > 0) & ~isload_iso(gen) )
    onld = find( (gen[:, GEN_STATUS] > 0) &  isload_iso(gen) )
    ...
    if OUT_AREA_SUM:
        ...
        for i in range(len(s_areas)):
            ...
            ig = find((bus[e2i[gen[:, GEN_BUS].astype(int)], BUS_AREA] == a) & ~isload_iso(gen))
            igon = find((bus[e2i[gen[:, GEN_BUS].astype(int)], BUS_AREA] == a) & (gen[:, GEN_STATUS] > 0) & ~isload_iso(gen))
            ildon = find((bus[e2i[gen[:, GEN_BUS].astype(int)], BUS_AREA] == a) & (gen[:, GEN_STATUS] > 0) & isload_iso(gen))
    ...
    if have_results_struct and 'userfcn' in results:
        if not isOPF:  ## turn off option for all constraints if it isn't an OPF
            ppopt = ppoption_iso(ppopt, 'OUT_ALL_LIM', 0)
        run_userfcn_iso(results["userfcn"], 'printpf', results, fd, ppopt)
