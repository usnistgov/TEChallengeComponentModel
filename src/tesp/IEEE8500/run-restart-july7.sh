...
xterm -fg green -bg black -l -lf $logs_directory/FileReader-${timestamp}.log -T "FileReader" -geometry 140x40+60+90 -e "sh run.sh conf/RestartJuly7.json" &
waitUntilJoined FileReader 1

cd $code_directory/GridModel
xterm -fg cyan -bg black -l -lf $logs_directory/GridModel-${timestamp}.log -T "GridModel" -geometry 140x40+90+120 -e "sh run.sh conf/RestartJuly7.json" &
waitUntilJoined GridModel 1

cd $code_directory/SimulationTime
xterm -fg white -bg black -l -lf $logs_directory/SimulationTime-${timestamp}.log -T "SimulationTime" -geometry 140x40+120+150 -e "sh run.sh conf/RestartJuly7.json" &
