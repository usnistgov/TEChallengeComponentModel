...
xterm -fg white -bg black -l -lf $logs_directory/federation-manager-${timestamp}.log -T "Federation Manager" -geometry 140x40+0+30 -e "export CPSWT_ROOT=`pwd` && mvn exec:java -P FederationManagerMatlab" &
...
xterm -fg green -bg black -l -lf $logs_directory/SimulationTime-${timestamp}.log -T "SimulationTime" -geometry 140x40+120+150 -e "sh run.sh" &
waitUntilJoined SimulationTime 1

cd $code_directory/MatlabClient
xterm -fg yellow -bg black -l -lf $logs_directory/MatlabClient-${timestamp}.log -T "MatlabClient" -geometry 140x40+150+180 -e "sh run.sh" &
