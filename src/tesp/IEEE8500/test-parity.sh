...
xterm -fg white -bg black -l -lf $logs_directory/federation-manager-${timestamp}.log -T "Federation Manager" -geometry 140x40+0+30 -e "export CPSWT_ROOT=`pwd` && mvn exec:java -P FederationManagerParity" &
...
xterm -fg yellow -bg black -l -lf $logs_directory/Controller-${timestamp}.log -T "Controller" -geometry 140x40+30+60 -e "sh run.sh" &
waitUntilJoined Controller 1

cd $code_directory/SimulationTime
xterm -fg white -bg black -l -lf $logs_directory/SimulationTime-${timestamp}.log -T "SimulationTime" -geometry 140x40+120+150 -e "sh run.sh" &
